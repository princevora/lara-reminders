...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="dark">
    ...
    <body class="min-h-screen bg-white dark:bg-zinc-800">
        <flux:sidebar sticky stashable class="border-r border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <a href="{{ route($dashboadRoute) }}" class="mr-5 flex items-center space-x-2" wire:navigate>
                <x-app-logo />
            </a>

            <flux:navlist variant="outline">
                <flux:navlist.group :heading="__('Platform')" class="grid">
                    <flux:navlist.item icon="home" :href="route($dashboadRoute)" :current="request()->routeIs($dashboadRoute)" wire:navigate>{{ __('Dashboard') }}</flux:navlist.item>
                </flux:navlist.group>
            </flux:navlist>

            @can('access-notification-types')
                <flux:navlist variant="outline">
                    <flux:navlist.group :heading="__('Notifications')" class="grid">
                        <flux:navlist.item icon="numbered-list" :href="route($dashboadRoute)" :current="request()->routeIs('dashboard')" wire:navigate>{{ __('Types') }}</flux:navlist.item>
                    </flux:navlist.group>
                </flux:navlist>
            @endcan
